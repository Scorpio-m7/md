...
但是，要运行至此处，要先满足 if ( *a1 == a1[1] )，a1是第一层的v4传入函数的形参，就是打印出来的地址。 a[0]=v4[0]=v3[0]=68 , a[1]=v4[1]=v3[1]=85 。要将a[0]和a[1]修改为相同的值。可以通过sub_400BB9();函数中printf(&format, &format); 格式化字符串任意写的漏洞来修改。函数sub_400BB9()内的v2是打印出来的v4的地址，我们需要知道v2在栈内的位置，这样才能通过 %?$n 向v2指向的地址处写入字符串长度。
...
考查一个备份文件，正常的备份一般是加.bak，然后访问ip/index.php/.bak就可以下载到备份文件，查看文件拿到flag
...
访问网页检查元素查看网络-cookie,看到cookie.php,然后直接访问

看到提示说查看response,响应消息头就藏着flag
...
有个按钮,查看源码可以发现是个input输入框,那直接post发送auth=flag数据就得到flag了或者在查看器中直接删掉disabled,这个按钮就可以按了,然后就出现flag了
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/2019042916415113.png)
...
查壳和程序的详细信息（D:\CTF\Reverse\ExeinfoPe\exeinfope.exe）

![img](https://adworld.xctf.org.cn/media/task/writeup/cn/logmein/1.png)

发现程序是一个ELF文件，将其放入Linux环境中进行分析

![img](https://adworld.xctf.org.cn/media/task/writeup/cn/logmein/2.png)

发现程序是64位的，使用静态分析工具IDA进行分析

![img](https://adworld.xctf.org.cn/media/task/writeup/cn/logmein/3.png)

从main函数开始分析，使用F5查看伪代码

![img](https://adworld.xctf.org.cn/media/task/writeup/cn/logmein/4.png)
...
![img](https://adworld.xctf.org.cn/media/task/writeup/cn/logmein/5.png)
...
如果想得到flag，则进入sub_4007f0函数

![img](https://adworld.xctf.org.cn/media/task/writeup/cn/logmein/6.png)
...
由于程序是小段的存储方式，所以，ebmarah就得变成harambe
...
if(first!=0xcafe){
    printf("youarewrong,sorry.\n");
    exit(2);
}
```

得出first=0xcafe转换成10进值为51966

```c
unsigned int second=atoi(argv[2]);
if(second%5==3||second%17!=8){
    printf("ha,youwon'tgetit!\n");
    exit(3);
}
```

得出（second%5!=3)and(second%17==8)-->second=25
...
在网站运行一下https://tool.lu/coderunner/ 上面的c脚本,或者下面的python脚本
...
附件下载时文本文档内容是：

base64decode->ascll->base64decode->ascll

**解题过程：**

base64：http://tool.oschina.net/encrypt?type=3
...
ascii(D:\CTF\Crypto\编码与密码\ASCII码随心换v3.0.0.1)10进值解出来:
...
先把所有的斜杠用空格代替,在解码莫斯,最后观察到是培根加密,进行解码即可得到ｆｌａｇ

**解题过程：**

摩斯电码解码：D:\CTF\Crypto\编码与密码\编码\摩斯电码编码解码.exe

摩斯密码解出来：may_be_have_another_decodehhhhaaaaabaabbbaabbaaaaaaaabaababaaaaaaabbabaaabbaaabbaabaaaababaabaaabbabaaabaaabaababbaabbbabaaabababbaaabbabaaabaabaabaaaabbabbaabbaabaabaaabaabaabaababaabbabaaaabbabaabba

培根密码(https://tool.bugku.com/peigen/)解出来:
...
附件下载时文本文档内容是：8842101220480224404014224202480122
...
jadx-gui(E:\实践\潍坊实训\安卓\工具\工具包\逆向分析\jadx_v1.0.0\lib\jadx-gui-1.0.0.jar)打开

源代码.com.droider.crackme0201.MainActivity
...
资源文件-resources.arsc-res-values-strings.xml中

11.<string name="successed">md5:b3241668ecbeb19921fdac5ac1aafa69</string>
...
1. F12检查源代码发现一句可疑注释
   ![在这里插入图片描述](https://img-blog.csdnimg.cn/20190506043357381.png)
2. 按照注释给出的参数去访问,得到一个空白的网页
   ![在这里插入图片描述](https://img-blog.csdnimg.cn/20190506043758848.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2FzZDQxMzg1MDM5Mw==,size_16,color_FFFFFF,t_70)
3. 随便输入参数的话,会显示文件不存在,证明刚才确实访问到了flag文件,现在要想办法拿到它的源代码。
   ![在这里插入图片描述](https://img-blog.csdnimg.cn/20190506043817599.png)
4. 接着检查网站,发现这样一个页面
   ![在这里插入图片描述](https://img-blog.csdnimg.cn/20190506044010344.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2FzZDQxMzg1MDM5Mw==,size_16,color_FFFFFF,t_70)
5. 访问.git目录,看来存在git源码泄露

![在这里插入图片描述](https://img-blog.csdnimg.cn/2019050604410315.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2FzZDQxMzg1MDM5Mw==,size_16,color_FFFFFF,t_70)
 \7. 用 GitHack获取到源码,然而flag文件是空的,那就分析index文件
 ![在这里插入图片描述](https://img-blog.csdnimg.cn/20190506044535104.png)
 \8. index.php中关键代码如下
...
9.page参数是GET请求获取的,而且没有做任何处理,可以用它构造代码让assert函数去执行,例如：**？page = '.system(“cd …/…/…/; ls -lA;”).'**
 这样$file最终会是 templates/‘system(“cd …/…/…/; ls -lA;”)’.php
 最终的执行结果如下：
 ![在这里插入图片描述](https://img-blog.csdnimg.cn/20190506045347278.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2FzZDQxMzg1MDM5Mw==,size_16,color_FFFFFF,t_70)
 但是为什么不会被strpos过滤掉呢？？？百思不得其解,哪位师傅能指点一下QAQ

10.直接cat获取flag内容即可（flag在注释里,要右键查看源代码才能看到）
 **?page=’.system(“cat ./templates/flag.php”).’** //文件结构在githack获取源码时得知
 ![在这里插入图片描述](https://img-blog.csdnimg.cn/2019050604563667.png)
 分析：对git的原理暂时还不熟,猜测应该是.git文件中记录的是之前的版本,所以flag文件是空的。官方wp说flag文件没有被创建,要用git diff恢复。不知道和我的方法原理是否相同。

一点疑惑：
 1、为什么构造的payload中含有..不会被strpos函数过滤掉
 2、在尝试cat flag之前,我尝试touch和mkdir,还有用cat写入文件内容。都没有效果,这是为什么呢,是因为用户权限不够吗？

> flag{y0uc4nc4l1me_eGl}
